/*
 * PreVision desktop launcher
 * ---------------------------
 * A tiny, dependency-free static file server for Windows.
 *
 * Why this exists: the original app is a Vite/React frontend with a small
 * Node/Express backend whose only job is to proxy a few public weather
 * APIs. The frontend already ships a client-side fallback
 * (src/utils/weatherClient.ts) that calls those same public APIs directly
 * from the browser whenever the backend route is missing or fails. That
 * means a native build doesn't need to reimplement the proxy at all: this
 * program only needs to serve the built dist/ folder over localhost, and
 * the app will transparently use the browser-side fallback for weather
 * data, exactly like it does today on static hosts (Netlify).
 *
 * Behavior:
 ...
 *   - Unmatched routes fall back to index.html (same as the original
 *     Express `express.static` + `app.get("*", ...)` behavior).
 */
...
int main(void) {
    printf("========================================\n");
    printf("  PreVision - Solar & Storage Sizing App\n");
    printf("========================================\n\n");
    ...
    if (attrs == INVALID_FILE_ATTRIBUTES) {
        ...
        printf("Make sure the 'dist' folder is next to PreVision.exe.\n");
        printf("Press Enter to exit...\n");
        getchar();
        return 1;
    }
    ...
    printf("Keep this window open while using PreVision.\n");
